...
NSTEMI_Mort <- NSTEMI %>%
  select(DCB_DES, Age, PMHx_PVD, PMHx_CVE, PMHx_MI, PMHx_PCI, PMHx_CABG, PMHx_HF, PMHx_AF, FHx_CAD, PMHx_COPD, PMHx_DM, GFR, LMS_VesselTx, Multivessel_VesselTx, Frailty_Sc, Heavy_Calc, Acuity_Sc, Mortality, Mortality_FU, CV_Mortality, CV_Mortality_FU, ACS, ACS_FU, MajBleeding, MajBleeding_FU, TIA, TIA_FU) %>%
  drop_na()
...
step_reg <- map(1:5, ~stats::step(res.cox[[.x]], direction='both', scope=list(lower = Surv(time = !!sym(Followups[.x]), event = !!sym(Outcomes[.x])) ~ DCB_DES, upper = stats::formula(res.cox[[.x]])), trace=0))
...
gtsummary::tbl_uvregression(NSTEMI_table,
                 method = glm,
...
NSTEMI_PS_Mort <- NSTEMI %>%
  select(DCB_DES, Age, Sex, PMHx_AF, FHx_CAD, Tash_Bif, Diffuse_Dx, Tortuosity, denovo_LesionLgth, denovo_VesselDiam, Mortality, Mortality_FU, Mortality_FU365, Mortality_365, Mortality_FU30, Mortality_30, CV_Mortality, CV_Mortality_FU, CV_Mortality_FU365, CV_Mortality_365, CV_Mortality_FU30,CV_Mortality_30, ACS, ACS_FU, ACS_FU365, ACS_365, ACS_FU30, ACS_30, TIA, TIA_FU, TIA_FU365, TIA_365, TIA_FU30, TIA_30, MajBleeding, MajBleeding_FU, MajBleeding_FU365, MajBleeding_365, MajBleeding_FU30, MajBleeding_30) %>%
  drop_na(1:10)

set.seed(1234)

match.it <- matchit(DCB_DES ~ Age+ Sex + PMHx_AF+ FHx_CAD+ Tash_Bif+ Diffuse_Dx+ Tortuosity+ denovo_LesionLgth+ denovo_VesselDiam, data = NSTEMI_PS_Mort, method="nearest", distance = "mahalanobis", replace = T)
a <- summary(match.it)
...
match.it <- matchit(DCB_DES ~ Age+ PMHx_PVD+ PMHx_CVE+ PMHx_MI+ PMHx_PCI+ PMHx_CABG+ PMHx_HF+ PMHx_AF+ FHx_CAD+ PMHx_COPD+ PMHx_DM+ GFR+ LMS_VesselTx+ Multivessel_VesselTx+ Frailty_Sc+ Heavy_Calc+ Acuity_Sc, data = NSTEMI_PS_Mort, method="nearest", distance = "mahalanobis", replace = T)
a <- summary(match.it)
