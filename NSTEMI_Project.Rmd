...
NSTEMI <- NSTEMI_S %>%
  transmute(Study_ID = as.numeric(STUDY_ID),
            ...
            CV_Mortality_FU = as.numeric(TimetoMortality),
...
plot6 <- ggsurvplot(fit6, fun = "cumhaz", pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, ylim=c(0,0.6), xscale="d_y", legend.labs = c("DCB", "DES"), xlab = "Years Since Index Procedure", ylab = "Probability of Incurring Composite Endpoint", palette = c("#fba30c", "#0077b8"), theme = "lancet", break.time.by	= 365.25, pval.coord = c(0,0.3), legend.title = "")
...
```{r Cum Haz Male Part 6 (all, 365, 30)}
NSTEMI_Sex <- NSTEMI %>%
  filter(Sex == "Male")
...
```{r Cum Haz Female Part 7 (all, 365, 30)}
NSTEMI_Sex <- NSTEMI %>%
  filter(Sex == "Female")
...
```{r Cum Haz Age >65 Part 8 (all, 365, 30)}
NSTEMI_Age <- NSTEMI %>%
  filter(Age >= 65)
...
```{r Cum Haz Age <65 Part 9 (all, 365, 30)}
NSTEMI_Age <- NSTEMI %>%
  filter(Age < 65)
